...
export function addAnzahlTastenToJson(jsonPath) {
  ...
  object.data.getProductListing.edges = object.data.getProductListing.edges.map(
    (edge) => {
      edge.node.AnzhalTatsen = edge.node?.Audio[0]?.features
        ? edge.node?.Audio[0]?.features[0]?.text
        : "";
      return edge;
    }
  );
...
export function extractNodes(clean) {
  return clean.data.getProductListing.edges.map((edge) => edge.node);
}
export function addAnzahlTastenToObject(object) {
  object.data.getProductListing.edges = object.data.getProductListing.edges.map(
    (edge) => {
      const AnzhalTatsen =
        edge.node?.Audio[0]?.features[0]?.text ||
        edge.node?.Audio[0]?.features[4]?.text ||
        null;
...
export function extractFrontplatteFeatures(pimcoreObjects) {
  const objects = pimcoreObjects.data.getProductListing.edges.map((edge) => {
    edge.node.Frontplatte[0].features =
      edge.node.Frontplatte[0].features?.filter((feature) => feature);
  });
  return objects;
}
export function extractDisplayEigenschaftenFeatures(pimcoreObjects) {
  const objects = pimcoreObjects.data.getProductListing.edges.map((edge) => {
    edge.node.DisplayEigenschaften[0].features =
      edge.node.DisplayEigenschaften[0].features?.filter((feature) => feature);
  });
  return objects;
}
