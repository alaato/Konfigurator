...
function addAsiPack(products: Pack[], video: boolean = false) {
	const packTCU: Pack = {
		station: { ...outdoorsStations.find((product) => product.MNR.includes(`ASI12000`)), quantity: 1 } as DeviceData,
		extension: {
			...outdoorsStations.find((product) => product.MNR.includes(`ASI21000-0000`)), quantity: 1
		} as DeviceData,
	};
	if(video){
		packTCU.camera ={...extensions.find((zubehoer) => zubehoer.MNR.includes(`FVK2202`)), quantity: 1} as DeviceData
		packTCU.controlUnit = {...units.find((unit) => unit.MNR.includes(`VBVS05`)), quantity: 1}
	}
	products.push(packTCU);
}

function addPesPackAudio(extButtonNumber, productsFilter: ProductsFilter, products: Pack[]) {
	const PETPES = outdoorsStations.find((product) => product.MNR == `PET${extButtonNumber}-EN/04`) as DeviceData
	const PETPESPRO = outdoorsStations.find((product) => product.MNR.includes(`AEA5302${extButtonNumber}`)) as DeviceData
	...
	const PES = outdoorsStations.find((product) => product.MNR.includes(`PES${pesModel}`)) as DeviceData
	const PESPRO = PETPESPRO && outdoorsStations.find((product) => product.MNR.includes(`${pesModel}`) && product.parent.MNR == "PES PRO") as DeviceData
...
function addPesPackVideo(extButtonNumber, productsFilter: ProductsFilter, products: Pack[]) {
	const PETPESPRO = outdoorsStations.find((product) => product.MNR.includes(`AEA5302${extButtonNumber}`)) as DeviceData
	const pesButtons = parseInt(productsFilter.AnzhalTatsen) - extButtonNumber;
	const evenPesButtons = pesButtons + pesButtons % 2
	const pesModel = evenPesButtons < 10 ? `0${evenPesButtons}` : evenPesButtons
	...
	const PESPRO = PETPESPRO && outdoorsStations.find((product) => product.MNR.includes(`AVA6102${pesModel}`) && product.parent.MNR == "PES PRO") as DeviceData
...
export function findOutDoorProductsWithEtexensions(productsFilter: ProductsFilter) {
	...
	else if (parseInt(productsFilter.AnzhalTatsen) > 32 && parseInt(productsFilter.AnzhalTatsen) <= 48 && productsFilter.funktion == "Video") addPesPackVideo(28,  productsFilter, products);
