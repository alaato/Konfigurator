<template>
	...
	<Card class="floor-card flex items-center gap-2 p-1 m-3 border-2">
		...
		<div ref="cardContent"
			class="flex apartment-container items-center gap-2 p-2 max-h-56 sm:w-96 sm:flex-wrap lg:flex-nowrap overflow-scroll">
		...
		<div class="overflow-y-auto border-l h-44">
			...
			<FloorOutdoorStation v-for="(outdoorStation, index) in floor.outdoorStations" :key="index" :index="index"
				:floorIndex="floorIndex" :outdoorStation="outdoorStation" />
		</div>
	</Card>
...
import { type Floor } from '~/utils/interfaces/interfaces';
